...
def default_run_id(args: argparse.Namespace) -> str:
    ...
    mtd_tag = format_param_token("mtd", str(args.null_method))
    rep_tag = format_param_token("rep", int(args.nb_rep))
    ...
    alpha_tag = format_param_token("p", f"{float(args.sm_alpha):g}")
    return "__".join(
        ["pfnull", mtd_tag, rep_tag, smooth_tag, xrem_tag, speed_tag, alpha_tag, ts]
    )
...
def main() -> None:
    ...
    classification_csv_cfg = str(args.classification_csv)
    class_path_cfg = Path(args.classification_csv)
    if not class_path_cfg.is_absolute():
        class_path_cfg = out_root / class_path_cfg
    cfg = {
        "script": "scripts/pipelines/build_placefield_null_from_interim.py",
        ...
        "classification_csv_resolved": str(class_path_cfg),
        "args": {
            ...
            "save_null_maps": bool(args.save_null_maps),
