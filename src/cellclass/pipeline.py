...
"""Feature extraction pipeline for one interim allcel NPZ session."""
...
from cellclass.processing import compute_acg, smooth_acg_gaussian
...
from cellclass.validation import validate_allcel_npz, validate_feature_table
...
def parse_session_id(npz_path: Path) -> Dict[str, str]:
    """
    Example filename:
      VS57_2022-12-18_18-51-04_Ratemap_final_these_allcel.npz
...
def extract_one(
    ...
    *,
    ...
    validate_allcel_npz(z, source=npz_path)
    ...
    # Session firing rate uses the spike-time span as the recording duration.
    ...
    acg_counts_smooth = smooth_acg_gaussian(
        acg_counts,
        sigma_ms=acg_smooth_ms,
        bin_ms=acg_bin_ms,
        axis=0,
    )
    ...
    # Refractory (Royer derivative method): use the raw ACG counts.
    ...
    # Peak latency as a rise-time proxy.
    ...
    validate_feature_table(df, source=f"features extracted from {npz_path}")
