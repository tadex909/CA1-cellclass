...
OutputFolder = '\\epsztein-nas02\team\Rachel\3_Adults\1_Binary\2_Clustered\RM02_2024-11-14_14-38-20\RM02_2024-11-14_14-38-20_Maps';
savepath = OutputFolder;

filesep = '/';
datalc.session_name = 'RM02_2024-11-14_14-38-20';
BaseFolder='\\epsztein-nas02\team\Rachel\3_Adults\1_Binary\2_Clustered\RM02_2024-11-14_14-38-20\';

mypath=[BaseFolder, '\',datalc.session_name,'_ePhy.mat'];
load(mypath)
load([BaseFolder, '\',datalc.session_name,'_TrajData.mat'])
load([BaseFolder, '\',datalc.session_name,'_Phenosys.mat'])
...
% Cond2 = find([Traj(:).Cond]==1);
% cond(Cond2)=2;
way=(strcmp({Traj.LR},'LR' )).';
back=(strcmp({Traj.LR}, 'RL')).';
...
prm.rmap.nb_cond = length(unique([Traj.Cond]))*2;
prm.rmap.freq = 1000;
prm.rmap.ismooth = 7;

prm.rmap.frmax = 1; %1.5
prm.rmap.frmean = 0.3; %0.5
prm.rmap.pctspklap = 0.50;

prm.pf.mtd = 'random';

prm.pf.nb_rep = 1000; %500
...
prm.pf.pval_crit = 0.01;
prm.pf.pval_edge_pf = 0.30;
...
prm.cellclass.frmaxinterneuron = 20;
prm.cellclass.duration = 0.4;

maxc = length(unique([Traj.Cond]));%attention la couille est là être sur qu'on en a 5 ou 6


%%%%IMPORTANT%%%%%%
%%%%NB of condition%%%%%%
eprm.nb_cond = length(unique([Traj.Cond]));
%.nb_cond = nbsess;

NbCell = length(allcel.id_cel);

for g = 1:NbCell
    disp(['analyzing cell #', num2str(g,'%d')])
    idx_spk = allcel.itime_spk(allcel.id_spk == allcel.id_cel(g)+1); %careful, id_cel starts at 0; is_spk starts at one
    idx_spk = fct_ifreq_swap(idx_spk,bhv.prm.freq,1000);
    [rmap(g),pf(g)] = fct_mapandfields(X,idx_spk,prm);
...
fct_plot_rmap(rmap,pf,'save',savepath)
%}
%%%%%%%%%


[allrmap, allpf] = fct_mapandfields_reshape(rmap, pf);  %Détection PF
...
% Spatial correlation avant/après muscimol : on corrèle allers sans muscimol avec allers sous muscimol, de même pour les retours 
...
%% AutoCorrelogram
% allcel.itime_spk = allcel.itime_spk./25000;% modif by PPLS


 [allcel.acg10_bin_u, allcel.acg10_window] = ACG(allcel.itime_spk, allcel.id_spk, allcel.id_cel+1, bhv.prm.freq, 10, 700);% allcel.id_cel+1: modif by ppls, not good for vinca
 [allcel.acg1_bin_u, allcel.acg1_window] = ACG(allcel.itime_spk, allcel.id_spk, allcel.id_cel+1, bhv.prm.freq, 1, 50);
...
[allcel.duration_u, allcel.asymmetry_u, allcel.peaktrough_u] = WaveformFeature(allcel.waveform, allcel.meanwaveform, bhv.prm.freq);  
...
crit_pf_stable_uc = allpf.feat.ispf_ucf(:, :, 1);% issue when 6 conditions, this one wants 5 only
...
% % maxc = floor(size(crit_pf_stable_uc, 2) / 2);  % safely define the number of usable conditions
% 
% for c = 1:eprm.nb_cond
%     ind_c = (2*(c-1) + 1):(2*(c-1) + 2);
% 
%     % Apply only if indices are valid
%     % if ind_c(2) <= size(crit_pf_stable_uc, 2)
%         crit_active_uc(:, c)   = (sum(allcel.state_uc(:, ind_c), 2) >= 1);
%         crit_inactive_uc(:, c) = (sum(allcel.state_uc(:, ind_c), 2) == 0);
% 
%         crit_bid_uc(:, c)    = crit_pf_stable_uc(:, ind_c(1)) & crit_pf_stable_uc(:, ind_c(2));
%         crit_uni_uc(:, c)    = xor(crit_pf_stable_uc(:, ind_c(1)), crit_pf_stable_uc(:, ind_c(2)));
%         crit_nonsm_uc(:, c)  = ~crit_pf_stable_uc(:, ind_c(1)) & ~crit_pf_stable_uc(:, ind_c(2));
% 
%         fr_c =  nanmean(allrmap.feat.fr_uc(:, ind_c), 2);
%         allrmap.feat.fr_c = [allrmap.feat.fr_c fr_c];
%     % end
% end
...
% Recalcul nombre cellules -> différencier cellules inactives des cellules que l'on perd (pyr_off/itn_off)
...
%si nécessaire : crit_present_cell_uc=[crit_present_cell_uc, zeros(30,3)]
...
allcel.nb_cel_on = mean(allcel.nb_cel_uc) - sum(allcel.pyr_off_uc + allcel.itn_off_uc); %allcel.nb_cel = nb cell enregistrées sur tout l'enregistrement
