    ...
    for c = 1:nb_cond
             if ispyrpftemp(u, c)  & c==1%si c une place cell 
                A{c} = [A{c} ; fr_s_cxutemp(c, : , u)]; %on  met dans A {field condition condw} le FR de condw c par bin pour la cellule n° u (taille 1x80)
                      bin_nb_pf(n,1:80)=zeros(1,80);
                      [seq1,seq2]=isonepf(find(ispf_cxutemp(c,:,u)))
                      
                if ~isempty(seq2) & length(seq2)>=length(seq1)

                    frmax=max(fr_s_cxutemp(c,seq2,u))
                    bin_nb_pf(n,find(fr_s_cxutemp(c,:,u)==frmax))=1

                    
                elseif  ~isempty(seq2) & length(seq2)<length(seq1)
                    frmax=max(fr_s_cxutemp(c,seq1,u))
                    bin_nb_pf(n,find(fr_s_cxutemp(c,:,u)==frmax))=1


                else

                     bin_nb_pf(n,round(mean(find(ispf_cxutemp(c,:,u)))))=1;
...
%                      bin_nb_pf_b(n,round(mean(find(ispf_cxutemp(c,:,u)))))=1;
%                 end
...
title(strcat('PO Aller Place cells Groupe',num2str(i)))
hold on
...
% plot(bin_allpf)
hold on
line([edgobj(1, 1) edgobj(1, 1)], [min(sumbin) max(sumbin)], 'color', 'm', 'linestyle', '--');
line([edgobj(1, 2) edgobj(1, 2)], [min(sumbin) max(sumbin)], 'color', 'm', 'linestyle', '--');
line([edgobj(2, 1) edgobj(2, 1)], [min(sumbin) max(sumbin)], 'color', 'm', 'linestyle', '--');
line([edgobj(2, 2) edgobj(2, 2)], [min(sumbin) max(sumbin)], 'color', 'm', 'linestyle', '--');
ylim=[0,1]
hold off
...
fct_plot_rankfield([A{1}], fprm)
%title(datalc.session_name, 'fontsize', 15)
title(strcat('PO Place cells Groupe',num2str(i)))
hold on
