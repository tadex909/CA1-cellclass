...
rmap.idx_spk_tx = cell(nb_tbin, 1);

for t = 1:nb_tbin
    idx_spk_t = idx_spk(idx_spk >= prm.tbin(t, 1) & idx_spk <= prm.tbin(t, 2));
    rmap.idx_spk_tx{t} = idx_spk_t;
    aa=find(idx_spk >= prm.tbin(t, 1) & idx_spk <= prm.tbin(t, 2));
    a=xpos(idx_spk_t);
    %b=fct_hist(a',prm.xbin);
    rmap.nbspk_tx(t, :) = fct_hist(xpos(idx_spk_t)', prm.xbin);
