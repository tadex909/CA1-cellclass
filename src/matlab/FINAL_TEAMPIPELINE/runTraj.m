function [Traj, newW]= runTraj (t, X, transition, W, starts, sessS, Speed, XSpeed,transitionThresh,AnlPath, OutpuFolder, sesNam)
Whl=W;
edgesX=[0:2.5:max(X)];
...
for  ss=1:length(Traj)
        if Traj(ss).Cond==1
        Traj(ss).icondway_tr=Traj(ss).Cond+(any(strcmp(Traj(ss).LR,'RL'))*2)-(1*any(strcmp(Traj(ss).LR,'RL')))
        elseif any(Traj(ss).LR=='LR')
             Traj(ss).icondway_tr=Traj(ss).Cond+(any(strcmp(Traj(ss).LR,'LR'))*2)-1
        else 
            Traj(ss).icondway_tr=Traj(ss).Cond+(any(strcmp(Traj(ss).LR,'RL'))*2)
        end
end
...
cd (AnlPath)
save('TrajData.mat','Traj','newW');% ask user input to re-write?
