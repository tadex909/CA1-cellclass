...
folder = 'X:\MATLAB\Data\Data_thèse';   % chemin du dossier
files = dir(fullfile(folder, '*_Ratemap_final_thèse.mat'));

for k =1:length(files)
    fnames = fullfile(folder, files(k).name);
    disp(['Chargement : ' fnames])
    load(strcat(fnames(1:end-24),'_TrajData.mat'));   % charge le .mat
     load(strcat(fnames(1:end-24),'_ePhy.mat'));
      load(strcat(fnames(1:end-24),'_Phenosys.mat'));
%       load(fnames)
    % ... ton traitement ici
loadcolmathSFN
load colormaprom.mat
Namesession=files(k).name(1:end-24);
savepath =folder;
...
%%%%%%%%%%%NEW NEW NEW NEW%%%%%%%%%%%%
prm.rmap.nb_cond = 10; %%%%NB : Mathilde était toujours sur 10 là dessus même si on avait que 2 ou 3 conditions. Si tu as au max 6 cond tu te mets à 12 (c'est ce que PP a fait)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
prm.pf.mtd = 'random'; %poisson %random
...
prm.pf.pval_edge_pf = 0.30; %OLD 0.30 augmentation taille pf pour vals adjacentes inf à 
...
for g = 1:NbCell
    ...
    %%%%%%%%%%%%%%%%%%%%%NEW NEW NEW%%%%%%%%%%%%%%%%%%%%%%%%
    [rmap(g),pf(g)] = fct_mapandfields(bhv.p_x_ds,idx_spk,prm);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% 
%     % --- Appel de la fonction de fit (définie plus bas dans ton script ou dans un fichier séparé) ---
...
% nb_true = sum(conds, 2);
% % critère : au moins 3 conditions vraies
...
%%%%%%NEW NEW NEW%%%%%%%%%%%%%%
%%%%%%Add BHV%%%%%%%%%%%%%%%%%%%
save([savepath filesep datalc.session_name '_Ratemap_final_CASE1'], 'allrmap', 'allpf', 'allcel', 'rmap', 'pf','bhv');
...
%% --- Fonction utilitaire pour calculer l’autocorrélogramme ---
function [acf, lags] = autocorr_spikes(spike_times, binSize, maxLag)
    ...
    for i = 1:length(spike_times)
        ...
        diffs(abs(diffs) < binSize) = []; % supprime le zéro central
...
% FIT_ACG_RISETIME3_SHIFT - Fit de l'autocorrélogramme d'un train de spikes
...
%
% Sorties :
%   beta      : paramètres du fit (1x5)
%   x_fit     : axe temporel du fit
%   y_fit     : données normalisées (partie positive du lag)
%   y_model   : modèle ajusté
%   t0_auto   : temps de début de montée détecté (s)
%   tau_rise  : temps de montée 63% (ms)
    ...
    % --- 2. Calcul de l'autocorrélogramme ---
    ...
    x = lags(idx);
    y = acf(idx);
    y = y / max(y); % normalisation à 1

    % --- 3. Détection automatique du t0 ---
    y_smooth = smoothdata(y, 'gaussian', 5); % lissage léger pour éviter le bruit
    ...
    if ~isempty(idx_above_threshold)
        ...
        % interpolation linéaire entre les deux points autour du 63%
        ...
        % sécurité : valeurs négatives ou aberrantes mises à NaN
    ...
    if showPlot
        ...
        xlabel('Lag (ms)');
        ylabel('Autocorrélogramme normalisé');
        title('Fit de l''ACG avec t_0 et \tau_{rise}');
        legend({'ACG réel', 'Fit', 't_0', '\tau_{rise}'}, 'Location', 'best');
        grid on; hold off;
    end
end
