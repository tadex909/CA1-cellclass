...
if isfield(rmap, 'idx_spk_tx')
    idx_spk_tx = rmap.idx_spk_tx;
else
    idx_spk_tx = cell(nb_tbin, 1);
end
tbin = prm.tbin;
xbin = prm.xbin;
ismooth = prm.ismooth;
...
%for t = 1:nb_tbin
    ...
    switch mtd
        ...
        case 'circular_shift'
            if isempty(idx_spk_tx) || length(idx_spk_tx) < t
                error('mtd=circular_shift requires rmap.idx_spk_tx (add with updated fct_rmap).');
            end
            idx_spk_t = idx_spk_tx{t};
            if isempty(idx_spk_t) || nbspk == 0
                nbspk_repx = zeros(nb_rep, nb_xbin);
            else
                % Keep only spikes that contributed to the current ratemap support.
                isvalid_spk = ~isnan(xpos(idx_spk_t)) & xpos(idx_spk_t) >= xbin(1) & xpos(idx_spk_t) <= xbin(end);
                idx_spk_t = idx_spk_t(isvalid_spk);
                
                if isempty(idx_spk_t)
                    nbspk_repx = zeros(nb_rep, nb_xbin);
                else
                    [is_in_valid, iloc_spk] = ismember(idx_spk_t, idxnonan);
                    iloc_spk = iloc_spk(is_in_valid);
                    
                    if isempty(iloc_spk)
                        nbspk_repx = zeros(nb_rep, nb_xbin);
                    else
                        if N > 1
                            delta_rep = randi(N - 1, nb_rep, 1); % avoid trivial zero shift
                        else
                            delta_rep = zeros(nb_rep, 1);
                        end
                        
                        iloc_rep = repmat(iloc_spk(:)', nb_rep, 1);
                        iloc_shift = mod(iloc_rep - 1 + repmat(delta_rep, 1, size(iloc_rep, 2)), N) + 1;
                        xpos_shift = xpos(idxnonan(iloc_shift));
                        
                        nbspk_repx = fct_hist(xpos_shift, xbin);
                    end
                end
            end
        otherwise
            error('Unknown placefield simulation method: %s', mtd);
    end
