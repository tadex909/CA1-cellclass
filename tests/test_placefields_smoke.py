...
class PlacefieldsSmokeTest(unittest.TestCase):
    ...
    def test_min_dwell_masks_low_occupancy_rates_only(self) -> None:
        position_x = np.array([0.2, 0.8, 1.2, 1.8, 0.3, 0.9, 1.4, 1.9], dtype=np.float64)
        spike_indices_0b = np.array([1, 2, 5, 6], dtype=np.int64)
        spike_cell_ids = np.array([10, 10, 10, 10], dtype=np.int64)
        cell_ids = np.array([10], dtype=np.int64)
        ...
        self.assertTrue(np.all(pack.dwell_tx_x < 3.0))
        self.assertTrue(np.all(np.isfinite(pack.nbspk_tx_ux)))
        self.assertTrue(np.all(np.isnan(pack.fr_tx_ux)))
        self.assertTrue(np.all(np.isnan(pack.fr_s_tx_ux)))

    def test_min_dwell_must_be_nonnegative(self) -> None:
        cfg = RatemapConfig(min_dwell_s=-0.1)
